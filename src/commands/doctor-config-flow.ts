...
/**
 * Discord user/role IDs are 64-bit snowflake IDs that exceed JavaScript's MAX_SAFE_INTEGER (2^53-1).
 * When parsed as JSON numbers, they lose precision. This function scans for numeric values in
 * Discord ID fields and returns warnings for users to convert them to strings.
 */
export type DiscordNumericIdHit = { path: string; value: number };

export function scanDiscordNumericIds(parsed: unknown): DiscordNumericIdHit[] {
  const hits: DiscordNumericIdHit[] = [];

  const scanArray = (pathLabel: string, arr: unknown) => {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (typeof entry === "number") {
        hits.push({ path: `${pathLabel}[${i}]`, value: entry });
      }
    }
  };

  const scanDiscordDm = (prefix: string, dm: unknown) => {
    if (!dm || typeof dm !== "object" || Array.isArray(dm)) {
      return;
    }
    const dmRec = dm as Record<string, unknown>;
    scanArray(`${prefix}.dm.allowFrom`, dmRec.allowFrom);
    scanArray(`${prefix}.dm.groupChannels`, dmRec.groupChannels);
  };

  const scanDiscordExecApprovals = (prefix: string, execApprovals: unknown) => {
    if (!execApprovals || typeof execApprovals !== "object" || Array.isArray(execApprovals)) {
      return;
    }
    const eaRec = execApprovals as Record<string, unknown>;
    scanArray(`${prefix}.execApprovals.approvers`, eaRec.approvers);
  };

  const scanDiscordGuild = (prefix: string, guild: unknown) => {
    if (!guild || typeof guild !== "object" || Array.isArray(guild)) {
      return;
    }
    const guildRec = guild as Record<string, unknown>;
    scanArray(`${prefix}.users`, guildRec.users);
    scanArray(`${prefix}.roles`, guildRec.roles);

    const channels = guildRec.channels;
    if (channels && typeof channels === "object" && !Array.isArray(channels)) {
      for (const channelId of Object.keys(channels as Record<string, unknown>)) {
        const channel = (channels as Record<string, unknown>)[channelId];
        if (!channel || typeof channel !== "object" || Array.isArray(channel)) {
          continue;
        }
        const channelRec = channel as Record<string, unknown>;
        scanArray(`${prefix}.channels.${channelId}.users`, channelRec.users);
        scanArray(`${prefix}.channels.${channelId}.roles`, channelRec.roles);
      }
    }
  };

  const scanDiscordAccount = (prefix: string, account: unknown) => {
    if (!account || typeof account !== "object" || Array.isArray(account)) {
      return;
    }
    const rec = account as Record<string, unknown>;
    scanArray(`${prefix}.allowFrom`, rec.allowFrom);
    scanDiscordDm(prefix, rec.dm);
    scanDiscordExecApprovals(prefix, rec.execApprovals);

    const guilds = rec.guilds;
    if (guilds && typeof guilds === "object" && !Array.isArray(guilds)) {
      for (const guildId of Object.keys(guilds as Record<string, unknown>)) {
        const guild = (guilds as Record<string, unknown>)[guildId];
        scanDiscordGuild(`${prefix}.guilds.${guildId}`, guild);
      }
    }
  };

  const scanDiscord = (discord: unknown) => {
    if (!discord || typeof discord !== "object" || Array.isArray(discord)) {
      return;
    }
    const rec = discord as Record<string, unknown>;

    // Scan top-level Discord config
    scanArray("channels.discord.allowFrom", rec.allowFrom);
    scanDiscordDm("channels.discord", rec.dm);
    scanDiscordExecApprovals("channels.discord", rec.execApprovals);

    // Scan guilds at top level
    const guilds = rec.guilds;
    if (guilds && typeof guilds === "object" && !Array.isArray(guilds)) {
      for (const guildId of Object.keys(guilds as Record<string, unknown>)) {
        const guild = (guilds as Record<string, unknown>)[guildId];
        scanDiscordGuild(`channels.discord.guilds.${guildId}`, guild);
      }
    }
    ...
    if (accounts && typeof accounts === "object" && !Array.isArray(accounts)) {
      for (const accountId of Object.keys(accounts as Record<string, unknown>)) {
        const account = (accounts as Record<string, unknown>)[accountId];
        scanDiscordAccount(`channels.discord.accounts.${accountId}`, account);
      }
    }
  };
  ...
  const channelsRec = channels as Record<string, unknown>;
  const discord = channelsRec.discord;
  if (discord) {
    scanDiscord(discord);
  }
...
export function noteDiscordNumericIdWarning(hits: DiscordNumericIdHit[]) {
  if (hits.length === 0) {
    return;
  }

  const examples = hits.slice(0, 3).map((h) => `${h.path}: ${h.value}`);
  const moreCount = hits.length > 3 ? hits.length - 3 : 0;

  const lines = [
    `- Discord user/role IDs must be strings, not numbers.`,
    `- Discord IDs are 64-bit snowflakes that exceed JavaScript's MAX_SAFE_INTEGER.`,
    `- Numeric values lose precision when parsed (e.g., 233734246190153728 becomes 233734246190153730).`,
    `- Found ${hits.length} numeric ID(s): ${examples.join(", ")}${moreCount > 0 ? ` (+${moreCount} more)` : ""}`,
    `- Fix: quote the values in openclaw.json (e.g., "233734246190153728" instead of 233734246190153728).`,
  ];

  note(lines.join("\n"), "Discord ID precision");
}
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  // Scan for numeric Discord IDs (issue #16681)
  // Discord user/role IDs are 64-bit snowflakes that exceed JavaScript's MAX_SAFE_INTEGER.
  // We must scan snapshot.parsed (the raw JSON) because numeric values lose precision during parsing.
  const discordNumericIdHits = scanDiscordNumericIds(snapshot.parsed);
  if (discordNumericIdHits.length > 0) {
    noteDiscordNumericIdWarning(discordNumericIdHits);
  }
